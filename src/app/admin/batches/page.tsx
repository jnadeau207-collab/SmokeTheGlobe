        ...
        <>
            ...
            <div className="flex items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-slate-400">
                        Create and manage batch records, link them to brands, and view aggregate ratings.
                    </p>
                </div>
                <button onClick={startCreate} className="px-3 py-1.5 rounded-md bg-emerald-500 text-slate-950 text-sm font-medium hover:bg-emerald-400">
                    + New batch
                </button>
            </div>

            {/* Batch Editor Form (shown when creating or editing a batch) */}
            {editing && (
                <div className="rounded-xl border border-slate-800 bg-slate-900/70 p-4 space-y-4">
                    ...
                    <div className="grid gap-3 md:grid-cols-2 text-sm">
                        ...
                        <div className="flex flex-col gap-1">
                            ...
                            <select
                                ...
                                onChange={e => setEditing({ ...editing, brandId: e.target.value ? Number(e.target.value) : null })}
                        ...
                        <div className="flex flex-col gap-1">
                            ...
                            <input
                                ...
                                onChange={e => setEditing({ ...editing, harvestDate: e.target.value || null })}
                        ...
                        <div className="flex flex-col gap-1">
                            ...
                            <input
                                ...
                                onChange={e => setEditing({ ...editing, productionDate: e.target.value || null })}
                                className="p-2 rounded-md bg-slate-900 border border-slate-700 text-slate-100"
                            />
                        </div>
                        <div className="flex flex-col gap-1">
                            ...
                            <input
                                ...
                                onChange={e => setEditing({ ...editing, packageDate: e.target.value || null })}
                                className="p-2 rounded-md bg-slate-900 border border-slate-700 text-slate-100"
                            />
                        </div>
                        <div className="flex flex-col gap-1">
                            ...
                            <input
                                ...
                                onChange={e => setEditing({ ...editing, expirationDate: e.target.value || null })}
                                className="p-2 rounded-md bg-slate-900 border border-slate-700 text-slate-100"
                            />
                        </div>
                    ...
                    <div className="flex items-center gap-2 pt-2">
                        <button onClick={saveBatch} disabled={saving} className="px-3 py-1.5 rounded-md bg-emerald-500 text-slate-950 text-sm font-medium hover:bg-emerald-400 disabled:opacity-60">
                            {saving ? "Saving..." : "Save batch"}
                        </button>
                        <button onClick={() => setEditing(null)} className="px-3 py-1.5 rounded-md border border-slate-700 text-sm text-slate-200">
                            Cancel
                        </button>
                    </div>
                </div>
            ...
            <div className="flex items-center justify-between gap-4">
                <input
                    ...
                    onKeyDown={e => { if (e.key === "Enter") fetchBatches(search); }}
                    className="flex-1 p-2 rounded-md bg-slate-900 border border-slate-700 text-sm text-slate-100 placeholder:text-slate-500"
                />
                <button onClick={() => fetchBatches(search)} className="px-3 py-1.5 rounded-md border border-slate-700 text-xs text-slate-200">
                    Apply
                </button>
            </div>

            <div className="grid gap-3">
                {loading && <div className="text-sm text-slate-400">Loading batches...</div>}
                {!loading && batches.length === 0 && (
                    <div className="text-xs text-slate-500">No batches found. Create your first batch above.</div>
                )}
                {!loading && batches.map(b => (
                    <div key={b.id} className="bg-slate-900/70 border border-slate-800 rounded-lg p-3 flex justify-between items-center gap-4">
                        <div className="space-y-1">
                            <div className="flex items-center gap-2">
                                <div className="font-semibold text-slate-100">{b.productName || b.batchCode}</div>
                                {!b.isActive && (
                            ...
                            <div className="text-xs text-slate-400">
                                <span className="mr-2">
                                    <span className="font-mono text-slate-300">{b.batchCode}</span>
                                </span>
                                {b.primaryCategory && <span className="mr-2">{b.primaryCategory}</span>}
                                {b.brand && <span className="mr-2 text-slate-300">· {b.brand.name}</span>}
                            </div>
                            <div>{renderRating(b)}</div>
                        </div>
                        <div className="flex gap-2">
                            <Link href={\`/admin/batches/\${b.id}\`} className="px-2 py-1 border border-slate-700 rounded text-xs text-slate-200">
                                View
                            </Link>
                            <button onClick={() => startEdit(b)} className="px-2 py-1 border border-slate-700 rounded text-xs text-slate-200">
                                Edit
                            </button>
                            <button onClick={() => deleteBatch(b.id)} className="px-2 py-1 border border-red-500/60 text-red-400 rounded text-xs">
                                Delete
                            </button>
                        </div>
                    </div>
